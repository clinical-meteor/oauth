...
OAuth.registerService = function (name, version, urls, handleOauthRequest) {
  console.log('C3.1  Performing the OAuth service registration...');

  if (registeredServices[name]) {
    console.log("Already registered the " + name + " OAuth service")
  } else {
    ...
    console.log('C3.2  Attached the new service to the registeredServices server object.')  
    process.env.DEBUG && console.log(registeredServices)  
  }
};
...
OAuth._generateState = function (loginStyle, credentialToken, redirectUrl) {
  let stateString = Buffer.from(JSON.stringify({
    loginStyle: loginStyle,
    credentialToken: credentialToken,
    redirectUrl: redirectUrl})).toString('base64');

    console.log('oauth_server:  Generating state string', stateString)

    return stateString;
};

// generate state from query
OAuth._stateFromQuery = function (query) {
  var string;
  try {
    string = Buffer.from(query.state, 'base64').toString('binary');
  } catch (e) {
    Log.warn('oauth_server:  Unable to base64 decode state from OAuth query: ' + query.state);
    throw e;
  }

  console.log('oauth_server:  Generating OAuth state string from query: ' + string)

  try {
    return JSON.parse(string);
  } catch (e) {
    Log.warn('oauth_server:  Unable to parse state from OAuth query: ' + string);
    throw e;
  }
};
...
var ensureConfigured = function(serviceName) {
  ...
  console.log('S7. Ensuring the ' + serviceName + 'service is configured.')
  if (!ServiceConfiguration.configurations.findOne({service: serviceName})) {
    throw new ServiceConfiguration.ConfigError('oauth_server.js ensureConfigured()');
  }
};

var isSafe = function (value) {
  console.log('S15. Checking if the token and secret are safe to use...')
...
OAuth._renderOauthResults = function(res, query, credentialSecret) {
  process.env.DEBUG && console.log('S14. OAuth._renderOauthResults')
...
var renderEndOfLoginResponse = function (options) {
  process.env.DEBUG && console.log('S17. renderEndOfLoginResponse()');
  ...
  process.env.DEBUG && console.log('S18. template()', template);
  process.env.DEBUG && console.log('S18. __meteor_runtime_config__.ROOT_URL_PATH_PREFIX', __meteor_runtime_config__.ROOT_URL_PATH_PREFIX);
  process.env.DEBUG && console.log('S18. process.env.ROOT_URL_PATH_PREFIX', process.env.ROOT_URL_PATH_PREFIX);
  process.env.DEBUG && console.log('S18. process.env.ROOT_URL', process.env.ROOT_URL);
  ...
  process.env.DEBUG && console.log('S19. loginResultHtml', loginResultHtml)
...
OAuth._endOfLoginResponse = function (res, details) {
  process.env.DEBUG && console.log("S16. OAuth._endOfLoginResponse()");

  // what's up with these details?
  console.log('Received some sweet details.  May want to do something with query.code, query.state, and credentials.token', details)
  ...
  if (details.loginStyle === 'redirect') {
    ...
    process.env.DEBUG && console.log('redirectUrl', redirectUrl);

    var appHost = Meteor.absoluteUrl();

    process.env.DEBUG && console.log('appHost', appHost);
...
OAuth._requestHandlers['2'] = function (service, query, res) {
  process.env.DEBUG && console.log("S8. OAuth._requestHandlers['2']", service)
  ...
  if (!query.error) {
    ...
    // this secret is temporary and gets over-ridden later
    var credentialSecret = '1234567890ABCDEF';
